...
## 2024-12-07 - [HUDLocator Scanner Optimization: Deferring GUID deductions in Loot Scanner]
**Learning:** In UE4SS Lua loops, deduplicating loop items by allocating a string via `ToString()` on C++ property `GetFullName()` creates significant GC and reflection overhead when done unconditionally, even for entities that are not within distance.
**Action:** Always defer object deduplication that requires string conversions until *after* checking fast spatial exclusions (e.g., `IsWithinDistanceSq`). This short-circuits evaluation and skips the deduplication overhead for entities that will be ignored anyway.
## 2026-09-26 - Optimization of math.sqrt in FreeCam camera module
**Learning:** In UE4SS Lua loops handling continuous inputs (like camera movement), calculating vector lengths unconditionally via `math.sqrt` incurs unnecessary CPU cost when the vector is zero or below significance. Furthermore, boundary checks that compare against a radius can be performed entirely with squared distances, avoiding `math.sqrt` operations.
**Action:** Convert  and  calculations to use squared values first (, ), and only execute the expensive  call inside conditional blocks when those squared values exceed their required thresholds (e.g., actual movement occurred, or boundary limit exceeded).
## 2024-03-24 - Optimization of math.sqrt in FreeCam camera module
**Learning:** In UE4SS Lua loops handling continuous inputs (like camera movement), calculating vector lengths unconditionally via `math.sqrt` incurs unnecessary CPU cost when the vector is zero or below significance. Furthermore, boundary checks that compare against a radius can be performed entirely with squared distances, avoiding `math.sqrt` operations.
**Action:** Convert length and dist2D calculations to use squared values first (lengthSq, dist2DSq), and only execute the expensive math.sqrt call inside conditional blocks when those squared values exceed their required thresholds (e.g., actual movement occurred, or boundary limit exceeded).
