...
## 2024-08-11 - Unused Table Allocations in Scanner Loops
**Learning:** In high-frequency UE4SS Lua scanner loops (like iterating over thousands of Pals), declaring and populating tables that are never used downstream creates unnecessary Garbage Collection (GC) pressure and CPU overhead.
**Action:** Always scan for and remove unused variable allocations, especially tables and string concatenations, within tight execution loops to prevent GC thrashing.
